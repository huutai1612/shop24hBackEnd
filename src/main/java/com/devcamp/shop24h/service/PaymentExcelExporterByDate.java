...
public class PaymentExcelExporterByDate {
	private XSSFWorkbook workbook;
	private XSSFSheet sheet;
	List<GetPayment> payments;

	public PaymentExcelExporterByDate(List<GetPayment> payments) {
		super();
	...
	public void writeHeaderLine() {
		...
		int rowCount = 0;
		createCell(row, rowCount++, "Ngày", style);
	...
	private void writeDataLine() {
		...
		for (GetPayment payment : payments) {
			...
			int columnCount = 0;
			createCell(row, columnCount++ , payment.getPaymentDate().toString(), style);
